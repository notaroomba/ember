...
#define SSD1306 SSD1306_Disp

/* Page index used during DMA transfers */
...
void ssd1306_WriteCommand(uint8_t byte) {
    /* Use blocking mem write for commands so the bus is ready before starting DMA */
    HAL_I2C_Mem_Write(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, 0x00, I2C_MEMADD_SIZE_8BIT, &byte, 1, ssd1306_I2C_TIMEOUT);
}

// Send data (start DMA, fallback to blocking)
void ssd1306_WriteData(uint8_t* buffer, size_t buff_size) {
    /* Try DMA mem write; fallback to blocking mem write if DMA not available */
    if (HAL_I2C_Mem_Write_DMA(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, 0x40, I2C_MEMADD_SIZE_8BIT, buffer, buff_size) != HAL_OK) {
        if (HAL_I2C_Mem_Write(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, 0x40, I2C_MEMADD_SIZE_8BIT, buffer, buff_size, ssd1306_I2C_TIMEOUT) != HAL_OK) {
            /* Mark error state on failure */
            print("I2C Write Error");
            SSD1306_Disp.state = SSD1306_I2C_ERROR;
        } else {
            /* Blocking succeeded — chain next page manually */
...
#error "Only 32, 64, or 128 lines of height are supported!"
    ...
    // Ensure state is READY before starting DMA-based screen update
...
void ssd1306_UpdateScreen(void) {
    // Only start transfer when we are READY
...
/* Uppercase API wrapper returning previous state */
...
void HAL_I2C_MemTxCpltCallback(I2C_HandleTypeDef *hi2c)
{
    ...
    {
        /* Chain pages until done, then set READY */
        updateScreenPageIndex++;
        if (updateScreenPageIndex < SSD1306_PAGES)
        {
            ssd1306_UpdatePage(updateScreenPageIndex);
        }
        else
...
/* Keep master TX complete callback as wrapper in case master transmit was used */
void HAL_I2C_MasterTxCpltCallback(I2C_HandleTypeDef *hi2c)
{
    HAL_I2C_MemTxCpltCallback(hi2c);
}

/* Set error state on I2C error */
