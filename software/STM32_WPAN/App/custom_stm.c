...
typedef struct{
  ...
  uint16_t  CustomLedHdle;                  /**< LED_STATUS handle */
...
#define COPY_EMBER_STATUS_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_LED_STATUS_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x01,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   *                                2 for LED_STATUS +
   *                                1 for LED_STATUS broadcast property +
   *                              = 4
   *
   ...
   *  LED_STATUS
   */
  COPY_LED_STATUS_UUID(uuid.Char_UUID_128);
  ...
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : LED , handle = 0x%04x \n\r", CustomContext.CustomLedHdle);
  }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_LED:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomStatHdle, CustomContext.CustomLedHdle, SizeLed, pPayload);
