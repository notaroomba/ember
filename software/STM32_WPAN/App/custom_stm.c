...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomStatHdle;                    /**< EMBER_STATUS handle */
  uint16_t  CustomGoodHdle;                  /**< IS_GOOD handle */
...
#define COPY_EMBER_STATUS_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x40,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_IS_GOOD_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x41,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomGoodHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for EMBER_STATUS +
   *                                2 for IS_GOOD +
   *                              = 3
   *
  ...
  COPY_EMBER_STATUS_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomStatHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: STAT, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: STAT , handle = 0x%04x \n\r", CustomContext.CustomStatHdle);
  }

  /**
   *  IS_GOOD
   */
  COPY_IS_GOOD_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomStatHdle,
                          UUID_TYPE_128, &uuid,
                          SizeGood,
                          ...
                          &(CustomContext.CustomGoodHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : GOOD, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : GOOD , handle = 0x%04x \n\r", CustomContext.CustomGoodHdle);
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_GOOD:
      ret = aci_gatt_update_char_value(CustomContext.CustomStatHdle,
                                       CustomContext.CustomGoodHdle,
                                       0, /* charValOffset */
                                       SizeGood, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value GOOD command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value GOOD command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_GOOD:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomStatHdle, CustomContext.CustomGoodHdle, SizeGood, pPayload);
